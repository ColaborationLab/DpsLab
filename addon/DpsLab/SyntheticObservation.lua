...
function Observation.Validate(value)
  ...
  if not closed(value.identity, { observation_id=true, captured_at=true }) or not boundedToken(value.identity.observation_id, 80) or type(value.identity.captured_at) ~= "string" or #value.identity.captured_at > 32 then return false, "observationIdentityInvalid" end
...
DpsLabSyntheticObservation = {
  ...
  observation = { state = "synthetic_fixture", sample_window_seconds = 0, event_count = 0, byte_count = 256, signals = { damage_events = 0, incoming_damage_events = 0, healing_events = 0 } },
