...
class LiveAnalysisExportAddonTests(unittest.TestCase):
 def test_manual_export_has_no_chat_network_or_savedvariable_surface(self):
  ...
  self.assertEqual(2,lua.count('CreateFrame(')); self.assertNotIn('RegisterEvent',lua+module); self.assertNotIn('C_Timer',lua+module); self.assertNotIn('OnUpdate',lua+module)
